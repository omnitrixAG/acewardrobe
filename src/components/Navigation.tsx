...
const AnimatedNavLink = ({ href, children, onClick }: { href: string; children: React.ReactNode; onClick?: () => void }) => {
  ...
  if (isExternal) {
    return (
      <a href={href} className={linkClasses} target="_blank" rel="noopener noreferrer" onClick={onClick}>
        {innerContent}
      </a>
    );
  }
  ...
  return (
    <header 
      className={`fixed top-4 md:top-6 left-1/2 transform -translate-x-1/2 z-50
                 ...
                 px-5 md:px-8 py-2.5 md:py-3 backdrop-blur-md
                 ${headerShapeClass}
                 border border-border bg-background/90
                 w-[calc(100%-1.5rem)] md:w-auto md:min-w-[700px] lg:min-w-[850px]
                 transition-[border-radius,background,shadow] duration-300 ease-in-out
    ...
    >
      <div className="flex items-center justify-between w-full gap-x-6 md:gap-x-10">
        <div className="flex items-center flex-shrink-0">
          <a href="/" onClick={handleLogoClick} className="flex items-center gap-3 group">
            <img src={aceLogo} alt="Ace Wardrobe Logo" className="w-8 h-8 object-contain transition-all duration-300 group-hover:scale-110" />
        ...
        <div className="hidden lg:flex items-center gap-3 flex-shrink-0">
          <Tooltip>
            <TooltipTrigger asChild>
              <a href="https://wa.me/2347039178489?text=Hi%20Ace%20Wardrobe!%20I'd%20like%20to%20place%20an%20order" target="_blank" rel="noopener noreferrer"
                className="p-2 text-muted-foreground hover:text-primary transition-colors">
                <ShoppingBag size={20} />
              </a>
            </TooltipTrigger>
            <TooltipContent><p>Order via WhatsApp</p></TooltipContent>
          </Tooltip>
          <Button 
            ...
            asChild
          >
            <a href="https://wa.me/2347039178489?text=Hi!%20I'd%20like%20to%20book%20a%20private%20shopping%20appointment%20at%20Ace%20Wardrobe" target="_blank" rel="noopener noreferrer" className="whitespace-nowrap">
              Book Appointment
            </a>
          </Button>
        </div>

        <button className="lg:hidden flex items-center justify-center w-8 h-8 text-muted-foreground focus:outline-none" onClick={toggleMenu} aria-label={isOpen ? 'Close Menu' : 'Open Menu'}>
          {isOpen ? (
      ...
      <div className={`lg:hidden flex flex-col items-center w-full transition-all ease-in-out duration-300 overflow-hidden
        ...
        <nav className="flex flex-col items-center space-y-4 text-base w-full">
          {navLinks.map((link) => (
            <AnimatedNavLink key={link.label} href={link.href} onClick={() => setIsOpen(false)}>{link.label}</AnimatedNavLink>
          ))}
        </nav>
        <div className="flex flex-col items-center space-y-3 mt-4 w-full">
          ...
          <Button variant="solid" size="sm" className="w-full" asChild>
            <a href="https://wa.me/2347039178489?text=Hi!%20I'd%20like%20to%20book%20a%20private%20shopping%20appointment%20at%20Ace%20Wardrobe" target="_blank" rel="noopener noreferrer">
              Book Appointment
            </a>
          </Button>
        </div>
